...
func (rpc *EthRPC) getUncle(method string, params ...interface{}) (*Uncle, error) {
  response := new(proxyUncle)

  err := rpc.call(method, response, params...)
  if err != nil {
    return nil, err
  }
  uncle := response.toUncle()
  
  return &uncle, nil
}

// EthGetUncleByBlockHashAndIndex returns information about an uncle by block hash and uncle index position.
func (rpc *EthRPC) EthGetUncleByBlockHashAndIndex(hash string, index int) (*Uncle, error) {
  return rpc.getUncle("eth_getUncleByBlockHashAndIndex", hash, IntToHex(index))
}

// EthGetUncleByBlockNumberAndIndex returns information about an uncle by block number and uncle index position
func (rpc *EthRPC) EthGetUncleByBlockNumberAndIndex(number int, index int) (*Uncle, error) {
